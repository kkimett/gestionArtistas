...
5. Si vas a usar MariaDB, instala además `pip install -r requirements-mysql.txt`.
6. Copia `.env.example` a `.env` y completa:
   - `DJANGO_SECRET_KEY`
   - `DJANGO_DEBUG=False`
   - `DJANGO_ALLOWED_HOSTS`
   - `DJANGO_CSRF_TRUSTED_ORIGINS`
   - `DB_NAME`, `DB_USER`, `DB_PASSWORD`, `DB_HOST`, `DB_PORT`
7. Ejecuta migraciones con `python manage.py migrate`.
8. Crea el superusuario con `python manage.py create_admin`.
9. Ejecuta `python manage.py collectstatic --noinput`.
10. Configura el panel del hosting para que use `passenger_wsgi.py` como entry point si te lo pide.
